...
class CoursesDAO {
   ...
   public function byCourseID ($courseID) {
      $stmt = $this->db->prepare ("select CourseID, CourseName, EntryPointID, AccessFlags from Courses where CourseID = ?");
      $stmt->bind_param ("i", $courseID);
      $stmt->execute ();
      $stmt->bind_result ($results ["CourseID"], $results ["CourseName"], $results ["EntryPointID"], $results ["AccessFlags"]);
      $stmt->fetch ();
      $stmt->close ();
      return $results;
   }
   ...
   public function fetchAll () {
      $resultsList = array ();
      $results = array ();
      $query = "select CourseID, CourseName, EntryPointID, AccessFlags from Courses";
   ...
   public function search ($params, $postfix = "") {
      ...
      $query = sprintf ("select CourseID, CourseName, EntryPointID, AccessFlags from Courses where %s", implode (" and ", array_keys ($params)));
      $query .= ' ' . $postfix;
   ...
   public function insert ($data) {
      $stmt = $this->db->prepare ("insert into Courses (CourseName, EntryPointID, AccessFlags) values (?, ?, ?)");
      $stmt->bind_param ("sii", $data ["CourseName"], $data ["EntryPointID"], $data ["AccessFlags"]);
      $stmt->execute ();
   ...
   public function save ($data) {
      $stmt = $this->db->prepare ("update Courses set CourseName = ?, EntryPointID = ?, AccessFlags = ? where CourseID = ?");
      $stmt->bind_param ("siii", $data ["CourseName"], $data ["EntryPointID"], $data ["AccessFlags"], $data ["CourseID"]);
      $stmt->execute ();
      if ($stmt->affected_rows != 1) {
